  ...
  <div class="intemShow">
    ...
    <el-dialog title="人员信息" v-model="dialogFormVisible" width="50%">
      <el-form :model="form" size="medium" :rules="rules" ref="form">
        ...
        <el-form-item label="编号" :label-width="formLabelWidth" prop="itemId">
        ...
        <el-form-item label="报名表" :label-width="formLabelWidth" prop="typeid">
...
export default {
  ...
  methods: {
    ...
    itemexport() { //导出
      this.$http.get("/item/export/"+this.typeState, {
        responseType: 'arraybuffer'
      }).then((res) => {
        ElMessage({
          message: '导出成功！',
        ...
        fileDownload(res, '报名信息_' + new Date().toLocaleString() + '.xlsx')
      }).catch(() => {
    ...
    saveData(resetForm) { //用于数据的添加和更新
      ...
      this.$refs[resetForm].validate((valid) => {
        if (valid) {
          this.form.flagUP = true //是否为管理员进行的修改或添加
